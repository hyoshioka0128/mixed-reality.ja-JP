...
6. 次に、 **Cookiecutter**をインストールします。 ターミナルで、次の bash コマンドを実行します。
    ...
    >2. **VS Code ターミナル**を、Pythonのインストールに使用していたもの (特に、Python環境が既にマシンにインストールされている場合) つまり **Powershell** に切り替える必要がある場合があります。 ターミナルを開いた状態で、ターミナルの右側にドロップダウンメニューが表示されます。
     ![コンテナを作成する](images/AzureLabs-Lab313-24b.png) 
